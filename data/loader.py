...
def load_property_data(week: str, property_name: str) -> Dict[str, Any]:
    ...
    if not excel_files:
        ...
        for prop in all_properties:
            ...
            if prop.strip() == property_name or prop == property_name + " ":
                print(f"🔍 LOADING DATA: Exact match found! Using prop='{prop}'")
                excel_files = storage_service.list_files(week, prop)
                property_name = prop
                break
            
            # Try property config mapping
            from config.property_config import find_property_by_directory_name
            mapped_property = find_property_by_directory_name(prop)
            if mapped_property == property_name:
                print(f"🔍 LOADING DATA: Config mapping match found! Using prop='{prop}' for property='{property_name}'")
                excel_files = storage_service.list_files(week, prop)
                property_name = prop
                break
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        organized_data = {'raw_data': {}, 'actual_property_directory': property_name}
