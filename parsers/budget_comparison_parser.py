...
def parse_budget_comparison(file_path: str) -> Dict[str, Any]:
    ...
    if header_row is not None:
        ...
        # Extract budget data including description column (column 1)
        budget_data = []
        for i in range(header_row + 1, len(df_raw)):
            # Include description from column 1, then the rest of the data
            description = str(df_raw.iloc[i, 1]).strip() if len(df_raw.columns) > 1 else ''
            row_data = df_raw.iloc[i, :len(clean_headers)].fillna('').astype(str).tolist()
            if any(cell.strip() for cell in row_data) or description:  # Skip empty rows
                # Prepend description to the row data
                full_row = [description] + row_data
                budget_data.append(full_row)
        
        if budget_data:
            # Add description column to headers
            full_headers = ['Description'] + clean_headers
            budget_data_df = pd.DataFrame(budget_data, columns=full_headers)
