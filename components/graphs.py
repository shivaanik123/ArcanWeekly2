...
def render_graphs_section(historical_data: Dict[str, Any], property_name: str = "Property", comprehensive_data: Dict[str, Any] = None):
    ...
    # Check what columns we actually have from the comprehensive reports
    print(f"🔍 GRAPH DEBUG: All available columns: {list(df.columns)}")
    
    # The comprehensive parser should provide these three distinct columns:
    # occupancy_percentage, leased_percentage, projected_percentage
    
    # If we don't have the three separate columns, something is wrong with the data
    required_cols = ['occupancy_percentage', 'leased_percentage', 'projected_percentage']
    missing_cols = [col for col in required_cols if col not in df.columns]
    
    if missing_cols:
        print(f"❌ GRAPH ERROR: Missing columns from comprehensive reports: {missing_cols}")
        print(f"❌ GRAPH ERROR: Available columns: {list(df.columns)}")
        # Don't create fallbacks - show error instead
        st.error(f"Missing data columns: {missing_cols}. The comprehensive report may not be parsed correctly.")
        return
    
    # We have all three columns - use them as-is
    print(f"✅ GRAPH SUCCESS: Found all three columns in data")
    
    # Just clip to reasonable ranges
...
def render_occupancy_trends(df: pd.DataFrame, property_name: str):
    ...
    # Add projected area (lightest blue, top layer)
    fig.add_trace(
        go.Scatter(
            x=df['date'],
            ...
            name="Projected %",
            fill='tozeroy',
            fillcolor='rgba(147, 197, 253, 0.4)',  # Light blue with transparency
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="Leased %",
            fill='tozeroy',
            fillcolor='rgba(59, 130, 246, 0.6)',  # Medium blue with transparency
    ...
    # Add occupancy area (darkest blue, bottom layer)
    fig.add_trace(
        go.Scatter(
            x=df['date'],
            ...
            name="Occupancy %",
            fill='tozeroy',
            fillcolor='rgba(30, 64, 175, 0.8)',  # Dark blue with transparency
    ...
    fig.update_layout(
        ...
        margin=dict(l=60, r=20, t=40, b=60, autoexpand=False),  # Consistent margins
        legend=dict(
...
def render_interactive_maintenance_chart(comprehensive_data: Dict[str, Any], property_name: str, time_period: str = "3 Months"):
    ...
    # Create the maintenance chart (both series together)
    fig = go.Figure()
    
    # Work orders as bar chart (dark blue)
    fig.add_trace(go.Bar(
        x=filtered_df['date'],
        y=filtered_df['work_orders_count'],
        name='Work Orders',
    ...
    # Make ready units as bar chart (light blue)
    fig.add_trace(go.Bar(
        x=filtered_df['date'],
        y=filtered_df['make_readies_count'],
        name='Make Ready Units',
    ...
    fig.update_layout(
        ...
        paper_bgcolor='rgba(0,0,0,0)',
        font=dict(color='white'),
        margin=dict(l=60, r=20, t=60, b=60, autoexpand=False),
        showlegend=True,
        legend=dict(
            orientation="h",
            yanchor="bottom",
